...
def encode_pcom_message(msg, id_lookup_maps):
    """
    ...
    payload = struct.pack("<HHHHHBB", msg.msg_id, msg.from_, msg.to, serialize_response_code(msg, id_lookup_maps), msg.msg_status,
    ...
    if msg.format_string:
        payload += struct.pack(("%ds" % len(msg.format_string)).encode("utf8"), msg.format_string.encode("utf8"))
...
def serialize_response_code(message, id_lookup_maps):
    """
    ...
    if m_type in VALID_MSG_TYPES:
        ...
        if isinstance(code, str):
            if m_type == "COMMAND":
                map_to_use = id_lookup_maps.command_name_map
            elif m_type == "PROPERTY":
                map_to_use = id_lookup_maps.property_name_map
            elif m_type == "STREAM":
                map_to_use = id_lookup_maps.stream_name_map
        ...
        # Handle the case that we're sending a response down to a board.
        elif type(message.to) == int and type(message.msg_status) == int:
            # Translate the "IN_PROGRESS", "COMPLETE", etc into a number.
...
def get_sub_type(msg, category):
    ...
    if category == MessageCategory.Order:
        ...
        # Check if the response is to a property.
        if "PROPERTY" in msg.contents:
            return ResponseSubType.Property
        # Otherwise it has to be a command, commands are not required to have "RESULT"
        else:
...
def get_option(msg, cat, sub_type):
    ...
    elif cat == MessageCategory.Order_Response:
        if sub_type == ResponseSubType.Command:
            # Change logic to handle inprogress commands
            if msg.topics.get("MSG_STATUS", 'OK') == "PROGRESS":
                return ResponseCommandOption.Inprogress
            else:
                return ResponseCommandOption.Complete
        elif sub_type == ResponseSubType.Property and "PROPERTY" in msg.contents:
            if "VALUE" in msg.contents:
                return ResponsePropertyOption.Get_Response
            else:
                return ResponsePropertyOption.Set_Response
        raise Exception("Unable to pick correct category for response!")
...
def decode_pcom_message(binary_msg):
    ...
    # look if we have an attribute indicating this message has a timestamp
    if (msg.attributes & TIMESTAMP_ATTR_MASK) == TIMESTAMP_ATTR_MASK:
        #increase the header size by the 4 byte timestamp size
        header_size = PACKET_HEADER_SIZE + TIMESTAMP_SIZE  # add the timestamp size
        msg.timestamp, = struct.unpack("<I", binary_msg[PACKET_HEADER_SIZE:header_size])

    # Extract the format string
    format_string_end_index = bytes(binary_msg).find(b'\0', header_size)
    format_string = binary_msg[header_size:format_string_end_index+1]
    format_string_length = (format_string_end_index+1) - header_size
    ...
    msg.format_string, = struct.unpack(("<%ds" % format_string_length).encode("utf8"), format_string)
    ...
    receive_format = "<" + translate_fmt_str(msg.format_string.decode("utf8"), binary_msg[format_string_end_index+1:])
    receive_format = receive_format[:-1]
    ...
    msg.data = struct.unpack(receive_format.encode("utf8"), binary_msg[format_string_end_index+1:])

    # Remove null byte from all strings in data
    # print msg.data
    #msg.data = [s[:-1] if isinstance(s, str) and s.endswith('\x00') else s for s in msg.data]
    msg.data = [s[:-1].decode("utf8") if isinstance(s, bytes) and s.endswith(b'\x00') else s for s in msg.data]
...
def unstuff_packet(packet):
    ...
    json_msg = None if (is_ack or is_nak) else decode_pcom_message(bytes(data))
