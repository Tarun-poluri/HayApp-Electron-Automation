...
class USBASCIILineProtocol(ASCIILineProtocol):
    ...
    def _filter_ports(port_list, vendor_id, product_id, name_compiled_regex):
        ...
        filtered = [port for port in port_list if USBASCIILineProtocol._match_usb_port(port, vendor_id, product_id, name_compiled_regex)]
        # Map each matching port to its string path
        return [port.device for port in filtered]
