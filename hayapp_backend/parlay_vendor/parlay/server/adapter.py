...
class Adapter(object):
    ...
    """
    DEFAULT_ADAPTER = None  # default adapter to use for any connection if a custom one isn't specified
...
class PyAdapter(Adapter):
    ...
    def track_open_protocol(self, protocol):
        ...
        """
        will_add_protocol = True
        for opened_protocol in self.open_protocols:
            if str(protocol) == str(opened_protocol):
                will_add_protocol = False
                break
    ...
    def open_protocol(self, protocol_name, open_params):
        ...
        if protocol_name not in ProtocolMeta.protocol_registry:
            ...
            # remove anything from the dictionary that is an empty string
            open_params = dict((k,v) for k,v in list(open_params.items()) if v != "")
