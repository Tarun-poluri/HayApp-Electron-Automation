...
UDP_MULTICAST_PORT = 16185  # The port on that group
    ...
    def datagramReceived(self, datagram, address):
        ...
        try:
            ...
            if request.get("type", None) == "GET_PARLAY_INFO":
                ...
                self.transport.write(json.dumps(info).encode('utf-8'),
                                     (address[0], CONSUMER_RESPONSE_PORT))
...
class ParlayConsumer(DatagramProtocol):
    ...
    def _doRequest(self):
        ...
        self.transport.write(json.dumps(request).encode('utf-8'),
                             (UDP_MULTICAST_GROUP, UDP_MULTICAST_PORT))
    ...
    def print_output(self):
        ...
        # get them in a table
        table = [["name", "URL"]]
        table.extend([ (v, k) for k,v in list(self.found_hosts.items())])
        # print the table
