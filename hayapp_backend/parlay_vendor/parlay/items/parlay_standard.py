...
class ParlayStandardItem(ThreadedItem):
    ...
    def get_discovery(self):
        ...
        # get from parent
        discovery = ThreadedItem.get_discovery(self)
...
def parlay_command(_async=False, auto_type_cast=True, override_id=None):
    """
    ...
    :param override_id: Override the command ID
    """
    ...
    def decorator(fn):
        if _async:
            ...
            if inspect.isgeneratorfunction(fn):
                raise Exception("Do not use the 'yield' keyword in a parlay command without 'parlay_command(_async=True)' ")
            wrapper = run_in_thread(fn)

        wrapper._parlay_command = True
        wrapper._parlay_override_id = override_id
    ...
    **Example: How to interact with the instantiated item from a Parlay script**::
    ...
    def __init__(self, item_id=None, name=None, reactor=None, adapter=None, parents=None):
        ...
        for member_name in [x for x in dir(self) if not x.startswith("__")]:
            ...
            if callable(member) and hasattr(member, "_parlay_command") and member._parlay_command:
                ...
                # Add in a key for the overrided id reference.
                if member._parlay_override_id is not None:
                    self._commands[(member_name, member._parlay_override_id)] = member
                else:
                    self._commands[(member_name, member_name)] = member
                # get the overridden ID if set, otherwise ID=Name
    ...
    def _add_commands_to_discovery(self):
        ...
        if len(self._commands) == 0:
            ...
            command_ids = list(self._commands.keys())
            command_ids.sort()  # pretty-sort

            # add the command selection dropdown
            self.add_field("COMMAND", INPUT_TYPES.DROPDOWN, label='command', default=command_ids[0],
                           # NOTE: Command ID's can be the same as the name, only a hashable value is required.
                           dropdown_options=[(command_name, command_id) for command_name, command_id in command_ids],
                           dropdown_sub_fields=[self._commands[command_id_name_tuple]._parlay_sub_fields
                                                for command_id_name_tuple in command_ids])
    ...
    def on_message(self, msg):
        ...
        # Since commands can be either ID's or by name we need to check each key value in the array.
        method = self._search_for_command(command)
        if command is not None and method is not None:
            try:
                # remove 'self'
                arg_names = method._parlay_fn.__code__.co_varnames[1: method._parlay_fn.__code__.co_argcount]
                ...
                params = params[len(params) - len(defaults):]
                default_lookup = dict(list(zip(params, defaults)))
                for k,v in list(default_lookup.items()):
                ...
                try:
                    ...
                    kws = {k: method._parlay_arg_conversions[k](v) if k in method._parlay_arg_conversions
                                                                   else v for k, v in list(kws.items())}
    ...
    def _search_for_command(self, command):
        """
        Search the values in the dictionary by keys, where keys can potentially be tuples.
        :param command: The command name to search for through the _commands dictionary.
        :return: The command function object that was located at the given key.
        """
        for key, value in list(self._commands.items()):
            if type(key) in [list, tuple, dict] and command in key:
                return value
            elif command == key:
                return value
        return None
