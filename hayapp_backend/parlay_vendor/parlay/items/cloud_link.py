...
class CloudLinkSettings(object):
    ...
    CLOUD_SERVER_ADDRESS = "https://dev.parlay.cloud"
...
class CloudLink(parlay.ParlayCommandItem):
    ...
    base_link_uri = parlay.ParlayProperty(val_type=str, default=CloudLinkSettings.CLOUD_SERVER_ADDRESS+"/bridge")
    ...
    @parlay.parlay_command(_async=True)
    ...
    @parlay.parlay_command(_async=True)
    def connect_to_cloud_bridge(self):
        """
        Connect up to the cloud bridge with UUID uuid
        :param uuid: the UUID of this device. The uuid determines which bridge you are connected to
        :return:
        """
        ...
        yield self.get_bridge()
        # cool, now set up the websocket connection to that bridge
        reactor = self._adapter.reactor

        print(("attempting to connect to", self.bridge_uri))
        self.cloud_factory = CloudLinkWebsocketClientFactory(self, self.bridge_uri,  reactor=reactor)
        self.cloud_factory.protocol = CloudLinkWebsocketClient
        self.cloud_factory.continueTrying = True  # attempt to reconnect

        if self.bridge_uri.startswith("wss"):
        ...
        defer.returnValue(self.bridge_uri)
    ...
    def get_bridge(self):
        ...
        # get the randomly assigned bridge for my UUID
...
class CloudLinkWebsocketClient(WebSocketClientProtocol):
    """
    The websocket client that will bridge all messages between the broker and the cloud bridge
    """
    ...
    def __init__(self, adapter=None):
        ...
        #self._adapter.subscribe(self.send_message_to_cloud_bridge)

    def send_message_to_cloud_bridge(self, msg):
        """
    ...
    def onMessage(self, payload, isBinary):
        ...
        try:
            ...
            if msg["TOPICS"].get('type') == 'subscribe':
                self._adapter.subscribe(self.send_message_to_cloud_bridge, **(msg["CONTENTS"].get("TOPICS", {})))

            else:
                self._adapter.publish(msg, self.send_message_to_cloud_bridge)
