...
class NoSuperInitItem(base.BaseItem):
    ...
    def __del__(self):
        # we know if the parent __del__() is called, it will throw an AttributeError,
        # since this item didn't register with the adapter
        pass
...
class BaseItemTest(unittest.TestCase, AdapterMixin, ReactorMixin):
    ...
    def testTwoChildrenDiscovery(self):
        ...
        self.assertTrue({'INTERFACES': [], 'TYPE': 'BaseItem/object', 'NAME': '1-Parent Child',
                                            'CHILDREN': [], 'ID': '1-Parent Child'} in self.first_parent.get_discovery()["CHILDREN"])
        self.assertTrue({'INTERFACES': [], 'TYPE': 'BaseItem/object', 'NAME': '2-Parent Child',
